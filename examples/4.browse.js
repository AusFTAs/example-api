...
FTAAPI.registerSegment('4.browse.js', 4, function (FTA, SOURCE, LEVEL)
{
  return function (FTA)
  {
    function summary(hscode, country)
    {
      return '[' +
        FTA.hierarchy[hscode].countryAgreements.filter(ca => ca.country === country)
    ...
    {
      segment.lazyDetails(parent => parent.h2(`${FTA.uglifyHSCode(section.data.hscode, true)}: ${section.data.description} <small>${summary(section.data.hscode, FTA.targetCountry)}</small>`), parent =>
      {
        let sectionBody = parent.div().style('margin-left', '2em');
        section.children.forEach(chapter =>
        {
          sectionBody.lazyDetails(parent => parent.h3(`${FTA.uglifyHSCode(chapter.data.hscode, true)}: ${chapter.data.description} <small>${summary(chapter.data.hscode, FTA.targetCountry)}</small>`), parent =>
          {
            let chapterBody = parent.div().style('margin-left', '2em');
            chapter.children.forEach(heading =>
            {
              chapterBody.lazyDetails(parent => parent.h3(`${FTA.uglifyHSCode(heading.data.hscode, true)}: ${heading.data.description} <small>${summary(heading.data.hscode, FTA.targetCountry)}</small>`), parent =>
              {
